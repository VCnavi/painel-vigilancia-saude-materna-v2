...
infos_comp_bloco2 <- data.frame(
  valores_comp = c(
    ...
    round(100 - 100 * params$data2$tx_abortos_mil_mulheres_valor_medio_sus / params$data2_comp$tx_abortos_mil_mulheres_valor_medio_sus, 1),
    round(100 - 100 * params$data2$tx_abortos_mil_mulheres_valor_medio_ans / params$data2_comp$tx_abortos_mil_mulheres_valor_medio_ans, 1),
    round(100 - 100 * params$data2$tx_abortos_cem_nascidos_vivos_valor_medio_sus / params$data2_comp$tx_abortos_cem_nascidos_vivos_valor_medio_sus, 1),
    round(100 - 100 * params$data2$tx_abortos_cem_nascidos_vivos_valor_medio_ans / params$data2_comp$tx_abortos_cem_nascidos_vivos_valor_medio_ans, 1)
  ),
...
data.frame(
  "indicador" = c(
    ...
    "Valor médio da taxa de abortos inseguros por mil MIF (no SUS)",
    "Valor médio da taxa de abortos inseguros por mil MIF (na ANS)",
    "Valor médio da razão de abortos inseguros por 100 nascidos vivos (no SUS)",
    "Valor médio da razão de abortos inseguros por 100 nascidos vivos (na ANS)"
  ),
  
  "valor_observado" = c(
    paste0(data2_formatado$porc_menor20),
    paste0(data2_formatado$porc_mais_3pt, "%"),
    paste0(data2_formatado$tx_abortos_mil_mulheres_valor_medio_sus),
    paste0(data2_formatado$tx_abortos_mil_mulheres_valor_medio_ans),
    paste0(data2_formatado$tx_abortos_cem_nascidos_vivos_valor_medio_sus),
    paste0(data2_formatado$tx_abortos_cem_nascidos_vivos_valor_medio_ans)
  ),
  ...
  "valor_de_referencia" = c(
    ...
    paste0(max_min[3], data2_comp_formatado$tx_abortos_mil_mulheres_valor_medio_sus, " (média nacional)"),
    paste0(max_min[4], data2_comp_formatado$tx_abortos_mil_mulheres_valor_medio_ans, " (média nacional)"),
    paste0(max_min[5], data2_comp_formatado$tx_abortos_cem_nascidos_vivos_valor_medio_sus, " (média nacional)"),
    paste0(max_min[6], data2_comp_formatado$tx_abortos_cem_nascidos_vivos_valor_medio_ans, " (média nacional)")
  ),
...
<ul align='justify'>
  <li> A taxa de abortos inseguros por mil mulheres em idade fértil (MIF) expressa a ocorrência de abortos inseguros em uma população feminina em idade fértil (10 a 49 anos), enquanto a razão de abortos inseguros por 100 nascidos vivos expressa a probabilidade de uma gestação terminar em aborto inseguro ao invés de um nascimento vivo. A estimativa do número de abortos inseguros se baseia no número de internações por abortamento em serviços públicos e privados, com aplicação de fator de correção para internações por abortos espontâneos e para abortos inseguros que não necessitaram de internação.
 </li>
  <span style='display: block; margin-bottom: 5px;'> </span>
  <li> Tanto a taxa quanto a razão de abortos inseguros estão calculados para a população total e para a população atendida no SUS e na saúde suplementar (internações pagas por planos de saúde). Para ambos os indicadores, utilizamos como referência a média nacional. Valores elevados, acima da média nacional, indicam a ocorrência frequente de abortos inseguros e a necessidade de ampliação de serviços de planejamento reprodutivo e de atenção qualificada aos casos de abortamento.
 </li>
</ul>
</div>
...
<div style = 'font-weight: bold; text-align: left; padding-bottom: 5px'> Porcentagem de mulheres com oito ou mais consultas de pré-natal </div>
<ul align='justify'>
...
infos_comp_bloco6 <- data.frame(
  valores_comp = c(
    round(100 - 100 * params$data6$prop_mmg_int_publicas_sus / params$data6_comp$prop_mmg_int_publicas_sus, 1),
    round(100 - 100 * params$data6$prop_mmg_hipertensao_sus / params$data6_comp$prop_mmg_hipertensao_sus, 1),
    round(100 - 100 * params$data6$prop_mmg_hemorragia_sus / params$data6_comp$prop_mmg_hemorragia_sus, 1),
    round(100 - 100 * params$data6$prop_mmg_infeccao_sus / params$data6_comp$prop_mmg_infeccao_sus, 1),
    round(100 - 100 * params$data6$prop_mmg_uti_sus / params$data6_comp$prop_mmg_uti_sus, 1),
    round(100 - 100 * params$data6$prop_mmg_tmp_sus / params$data6_comp$prop_mmg_tmp_sus, 1),
    round(100 - 100 * params$data6$prop_mmg_transfusao_sus / params$data6_comp$prop_mmg_transfusao_sus, 1),
    round(100 - 100 * params$data6$prop_mmg_cirurgia_sus / params$data6_comp$prop_mmg_cirurgia_sus, 1)
  ),
  invertido = c(
    rep(FALSE, 8)
  ),
...
data.frame(
  "indicador" = tabela_indicadores$indicador[which(tabela_indicadores$bloco %in% c("bloco6_morbidade_sus"))],
  "valor_observado" = c(
    dplyr::if_else(is.nan(params$data6$prop_mmg_int_publicas_sus), "---", paste0(data6_formatado$prop_mmg_int_publicas_sus, "%")),
    dplyr::if_else(is.nan(params$data6$prop_mmg_hipertensao_sus), "---", paste0(data6_formatado$prop_mmg_hipertensao_sus, "%")),
    dplyr::if_else(is.nan(params$data6$prop_mmg_hemorragia_sus), "---", paste0(data6_formatado$prop_mmg_hemorragia_sus, "%")),
    dplyr::if_else(is.nan(params$data6$prop_mmg_infeccao_sus), "---", paste0(data6_formatado$prop_mmg_infeccao_sus, "%")),
    dplyr::if_else(is.nan(params$data6$prop_mmg_uti_sus), "---", paste0(data6_formatado$prop_mmg_uti_sus, "%")),
    dplyr::if_else(is.nan(params$data6$prop_mmg_tmp_sus), "---", paste0(data6_formatado$prop_mmg_tmp_sus, "%")),
    dplyr::if_else(is.nan(params$data6$prop_mmg_transfusao_sus), "---", paste0(data6_formatado$prop_mmg_transfusao_sus, "%")),
    dplyr::if_else(is.nan(params$data6$prop_mmg_cirurgia_sus), "---", paste0(data6_formatado$prop_mmg_cirurgia_sus, "%"))
  ),
  "valor_de_referencia" = c(
    paste0(max_min[1], data6_comp_formatado$prop_mmg_int_publicas_sus, "% (média nacional)"),
    paste0(max_min[2], data6_comp_formatado$prop_mmg_hipertensao_sus, "% (média nacional)"),
    paste0(max_min[3], data6_comp_formatado$prop_mmg_hemorragia_sus, "% (média nacional)"),
    paste0(max_min[4], data6_comp_formatado$prop_mmg_infeccao_sus, "% (média nacional)"),
    paste0(max_min[5], data6_comp_formatado$prop_mmg_uti_sus, "% (média nacional)"),
    paste0(max_min[6], data6_comp_formatado$prop_mmg_tmp_sus, "% (média nacional)"),
    paste0(max_min[7], data6_comp_formatado$prop_mmg_transfusao_sus, "% (média nacional)"),
    paste0(max_min[8], data6_comp_formatado$prop_mmg_cirurgia_sus, "% (média nacional)")
  ),
  "comparacao" = c(
    html_comp_bloco6[1],
    html_comp_bloco6[2],
...
if (params$nivel %in% c("macro", "micro")) {
  ...
  if (isTruthy(params$data_incompletude$idademae)) {
    infos_comp_imcompletude <- data.frame(
      valores_comp = c(
        ...
        round(100 - 100 * params$data_incompletude$racacormae / 5, 1),
  ...
  data.frame(
    "indicador" = c(
      ...
      "Porcentagem de RACACORMAE não preenchida ou ignorada",
    ...
    "valor_observado" = c(
      ...
      paste0(data_imcompletude_formatado$racacormae, "%"),
...
data.frame(
  "indicador" = c(
    ...
    "Porcentagem da variável RACACORMAE não preenchida ou ignorada",
